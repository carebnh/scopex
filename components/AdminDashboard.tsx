...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  ...
  const handleDelete = async (lead: any) => {
    ...
    if (success) {
      ...
      alert("Synchronization conflict. Could not delete record.");
    }
    setIsDeleting(false);
  };
      ...
      <div className="bg-white w-full h-full md:max-w-7xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden relative border border-white/20">
        ...
        <div className="px-8 py-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white shrink-0">
            ...
            <div>
              ...
              <div className="flex items-center space-x-3">
                 ...
                 <span className="text-[10px] font-black text-gray-400 tracking-widest uppercase">
                  Registry Sync: {data.length} Total Records
                 </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <button 
              onClick={loadData}
              className="p-4 bg-gray-50 text-scopex-blue hover:bg-scopex-blue hover:text-white rounded-2xl transition-all shadow-sm flex items-center space-x-2"
              title="Force Sync"
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">Sync Now</span>
            </button>
            <button 
              onClick={onClose}
        ...
        <div className="px-8 py-6 bg-gray-50/50 border-b border-gray-100 flex flex-col md:flex-row md:items-center gap-6 shrink-0">
          ...
          <div className="relative flex-1">
            <input 
              type="text" 
              placeholder="Search synchronized records by name or mobile..."
              value={searchQuery}
        ...
        <div className="flex-1 overflow-auto custom-scrollbar p-0 bg-white">
          {loading ? (
            <div className="h-full flex items-center justify-center flex-col space-y-4">
              ...
              <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">Accessing Registry Database...</p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead className="sticky top-0 bg-white z-10 shadow-sm">
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Registry Status</th>
                ...
                {filteredData.length > 0 ? filteredData.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-6">
                      ...
                      <p className="text-[10px] font-bold text-gray-300 uppercase tracking-tighter mt-0.5">{item.type === 'camp' ? 'Corporate Event' : 'Medical Inquiry'}</p>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-xl w-fit ${item.id.includes('local_') ? 'bg-orange-50 text-orange-500' : 'bg-scopex-green/10 text-scopex-green'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${item.id.includes('local_') ? 'bg-orange-400 animate-pulse' : 'bg-scopex-green'}`}></span>
                        <span className="text-[9px] font-black uppercase tracking-widest">
                          {item.id.includes('local_') ? 'Local Cache' : 'Live Sync'}
                        </span>
                      </div>
                    </td>
                    <td className="px-8 py-6" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          onClick={() => setSelectedLead(item)}
                          className="p-2.5 bg-scopex-blue text-white rounded-xl shadow-lg shadow-blue-900/10 active:scale-90"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                        </button>
                        <button 
                          onClick={() => handleDelete(item)}
                          className="p-2.5 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all active:scale-90"
                        >
                  ...
                  <tr>
                    <td colSpan={6} className="py-32 text-center flex flex-col items-center">
                       ...
                       <p className="text-xs font-black text-gray-300 uppercase tracking-[0.4em]">Waiting for Synchronized Records...</p>
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col border border-white/20 animate-in zoom-in duration-300">
              <div className="bg-scopex-blue p-10 text-white relative">
                ...
                <div className="flex items-center space-x-3 mt-4">
                  <span className="px-3 py-1 bg-white/10 rounded-full text-[9px] font-black uppercase tracking-widest border border-white/10">ID: {selectedLead.id}</span>
                  <span className="text-[9px] font-black text-blue-100/60 uppercase tracking-[0.2em]">{selectedLead.timestamp}</span>
                </div>
              </div>
              ...
              <div className="p-10 space-y-8 overflow-y-auto max-h-[50vh] custom-scrollbar">
                <div className="grid grid-cols-2 gap-x-8 gap-y-8">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Authority Name</p>
                    <p className="text-lg font-black text-slate-800 leading-tight">{selectedLead.contactName || selectedLead.fullName}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Mobile Contact</p>
                    <p className="text-lg font-black text-scopex-blue">{selectedLead.mobile || selectedLead.phone}</p>
                  </div>
                  {selectedLead.email && (
                    <div className="col-span-2 space-y-1">
                      <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Digital Channel</p>
                      <p className="text-sm font-bold text-slate-600 truncate">{selectedLead.email}</p>
                    </div>
                  )}
                  {selectedLead.requirements && (
                    <div className="col-span-2 p-8 bg-gray-50 rounded-[2.5rem] border border-gray-100 shadow-inner">
                      <div className="flex items-center space-x-2 mb-3">
                         <div className="w-1.5 h-1.5 rounded-full bg-scopex-blue"></div>
                  ...
                  {selectedLead.interest && (
                    <div className="col-span-2 space-y-1">
                      <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Operational Interest</p>
                      <p className="text-lg font-black text-scopex-green uppercase tracking-tighter">{selectedLead.interest}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-10 border-t border-gray-100 flex gap-4 bg-white/80">
                ...
                <button 
                  ...
                  disabled={isDeleting}
                  className="px-8 bg-red-50 text-red-500 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all disabled:opacity-50 active:scale-95"
                >
                  Delete
                </button>
              </div>
        ...
        <div className="px-8 py-5 border-t border-gray-100 bg-white flex items-center justify-between">
           <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Registry v2.1 • Authorized Management Access Only</p>
           <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Auto-Sync Active</p>
        </div>
      </div>
    </div>
  );
